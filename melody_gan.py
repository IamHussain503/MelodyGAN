...
class MelodyGAN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        """
        MelodyGAN model with architecture matching the checkpoint.

        Args:
            input_dim (int): Dimension of input features (e.g., 7).
            hidden_dim (int): Hidden layer dimension.
            output_dim (int): Output feature dimension.
        """
        super(MelodyGAN, self).__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dim)  # 1024 x 7
        self.fc2 = nn.Linear(hidden_dim, hidden_dim)  # 1024 x 1024
        self.fc3 = nn.Linear(hidden_dim // 2, hidden_dim)  # 512 x 1024
        self.fc4 = nn.Linear(hidden_dim // 2, output_dim * 3)  # 384 x 512
